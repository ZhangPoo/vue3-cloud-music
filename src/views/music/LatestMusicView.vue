...
<template>
    <div class="sticky top-0  z-10  p-4 bg-slate-50 dark:bg-black">
        <span v-for="item in typeList" :key="item.value"
            ...
            @mouseleave="handleMouseLeave($event, item.value)" @click="handleTypeClick(item.value)">{{ item.name
            }}</span>
    </div>
    <!-- 新歌速递列表 -->
    <div class="mt-4">
        ...
        <transition name="fade" appear mode="out-in">
            <ul v-show="!isLoading" class="songList">
                <li v-for="(item, index) in newSongList" :key="item.id"
                        ...
                        <p class="w-5 text-sm opacity-80">
                            {{ index < 9 ? '0' + (index + 1) : (index + 1) }} </p>
                                <div style="-webkit-transform: translateZ(0);"
                                    class="relative ml-4 w-16 h-16 rounded-md">
                                    <load-img loading-height="64px" class-name="w-16 h-16 rounded-md"
                                        :src="item.album.picUrl" :show-message="false" :double-click-preview="false" />
                    ...
                    <n-time class="pl-4 mx-2 text-sm text-left opacity-80" :time="item.bMusic?.playTime"
                        format="mm:ss" />
                </li>
            </ul>
        </transition>
